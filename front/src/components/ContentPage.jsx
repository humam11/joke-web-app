...
export default function ContentPage() {
  ...
  function getCategoryLabel(item) {
    return typeof item.category === 'string' ? item.category : item.category?.slug ?? 'general';
  }
      ...
      <div className="content-page__grid">
        {items.map((item) => (
          <article className="content-card" key={item.id}>
            <p className="content-card__meta">{getCategoryLabel(item)} · {item.content_type ?? item.type}</p>
